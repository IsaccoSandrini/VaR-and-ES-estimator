...
public class DataCollectionAndPlotting {
	...
	public static double[] getHistoricalPricesStock1() throws IOException {
		...
		try (InputStream is = DataCollectionAndPlotting.class.getResourceAsStream("/Stock1.xlsx");
		     Workbook workbook = new XSSFWorkbook(is)) {

			Sheet sheet = workbook.getSheetAt(0); // considera il primo foglio di Excel
			int columnIndex = 1; // colonna di Excel da considerare
		...
		return pricesList.stream().mapToDouble(Double::doubleValue).toArray(); // Restituisce l'array di prezzi su cui lavorare
	}
	
	public static double[] getHistoricalPricesStock2() throws IOException {
		List<Double> pricesList = new ArrayList<>();

		try (InputStream is = DataCollectionAndPlotting.class.getResourceAsStream("/Stock2.xlsx");
		     Workbook workbook = new XSSFWorkbook(is)) {
